...
- `decision.reference_bundle_drift_present`
- `decision.bootstrap_missing_present`
...
- `docs/` missing:
  - explain that the repository lacks a documentation surface
  - recommend creating at least an index entrypoint
- `docs/quickstart.md` missing or placeholder:
  - recommend a real first-run or onboarding guide
- `docs/INDEX.md` missing or placeholder:
  - recommend a canonical docs entrypoint with navigation
- `.specify/memory/constitution.md` missing or placeholder:
  - recommend restoring and tracking a real working agreement
- `.gitignore` missing:
  - keep severity high and recommend restoring it
- `.gitignore` policy violations:
  - explain each missing required ignore separately:
    - `.codex`
    - `.claude`
    - `.specify`
    - `specs/*`
  - if `.specify/memory/constitution.md` is ignored, explain why it must stay tracked
...
```json
{
  ...
  "analysis": [
    {
      "finding_id": "observed.docs_missing",
      "category": "observed",
      "path": "docs/",
      "severity": "warning",
      "state": "missing",
      "qualifies_for_issue": true,
      "summary": "Repository has no documentation entry surface.",
      "recommendation": "Add a docs index and link it from onboarding materials.",
      "evidence": [
        "doctor-findings.json: observed.docs_missing",
        "docs/ path is absent in the checked out repository"
      ]
    }
  ]
}
