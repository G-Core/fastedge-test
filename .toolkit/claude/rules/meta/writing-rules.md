...
doc_type: policy
lang: en
tags: ['rules', 'meta', 'guidelines', 'best-practices']
...
How to author rule files for the rules/ directory

TL;DR: One file = one topic. YAML frontmatter required (doc_type, tags, last_modified).
Use `paths:` in frontmatter to scope a rule to specific file patterns and save tokens.
Keep rules concise — every rule costs tokens every session it's loaded.
...
RULES/ VS DOCS/ — WHEN TO USE EACH
===================================

Not everything belongs in rules/. The decision heuristic:

- Behavioral ("do this when X", "never do Y") → rules/
- Informational ("here's how X works", "API reference") → docs/

Rules prescribe. Docs describe. If the content doesn't contain MUST/NEVER/ALWAYS or
workflow steps, it's probably a doc. Large reference material should always go to docs/.

For the full docs-authoring guide see [writing-docs.md](writing-docs.md).
...
See the symlink examples in `toolkit/scripts/`.
