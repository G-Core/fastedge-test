...
description: >-
  lint and optionally auto-fix markdown frontmatter against
  `.claude/rules/frontmatter.md`. use this skill when asked to check, lint,
  validate, normalize, or fix markdown frontmatter, yaml frontmatter,
  metadata fields, last_modified, copyright, doc_type, audience, lang, or
  tags in markdown files.
last_modified: 2026-03-10T09:31:35Z
...
Load `.claude/rules/frontmatter.md`.
Apply only the rules declared in that file; do not restate or invent extra rules.
Check only frontmatter ownership. Markdown-body findings are out of scope for this skill.
...
Default path: `.claude/rules/frontmatter.md`.
...
2. Parse these structures as needed for rule evaluation:
   ...
   - headings;
   - fenced code blocks and inline code, when needed for language detection;
...
Treat markdown-body ownership strictly:
- Do not report H1, TL;DR, code-fence, bash-block, or line-length findings with this skill.
- Body content may be used only when a frontmatter rule explicitly requires it, such as
  language or tag detection.
...
- Partial frontmatter:
  preserve already valid keys and repair only what the applicable rules allow.
- Malformed frontmatter:
  rebuild it only when the applicable frontmatter rule allows a deterministic rebuild.
  Otherwise report it as manual.
- Missing frontmatter:
  apply the applicable frontmatter schema rules and auto-fix only when the result is
  deterministic.
...
- Use final-file line numbers for manual actions when the location is still stable after
  fixes. Otherwise use one of these location labels: `frontmatter`, `file-wide`.
...
Do not report markdown-body issues such as H1, TL;DR, code fences, bash blocks, or line
length.
Do not silently ignore a frontmatter rule violation.
